...
- Status: **Proposal v3 (待讨论)**
- Date: 2026-07-21 (v3 updated — per-block heterogeneous compute + FLOPS)
- Scope: (A) 修正 FSDP 配置间内存估计无差异的问题（分析路径 + DES 路径）；
...
2. **SHARD_GRAD_OP 的 AG buffer = full chunk**：逐层累积。——已确认
