...
- 状态：**提案（待讨论）**
- 日期：2026-07-21
- 参考：[PyTorch FSDP2 教程](https://docs.pytorch.org/tutorials/intermediate/FSDP_tutorial.html)
- 范围：当前 layer-wise FSDP 建模与 FSDP2 实际行为的逐项比对，识别差异，
  设计修正方案。
...
| 阶段 | 操作 | 通信类型 |
|------|------|---------|
| Forward 前 | unshard 参数 | all-gather |
| Forward 后 | reshard 参数 | 无通信（释放显存） |
| Backward 前 | unshard 参数（仅 FULL_SHARD） | all-gather |
| Backward 内 | shard 梯度 | reduce-scatter |

### 1.2 Sharding Strategy

| 策略 | `reshard_after_forward` | Forward 后 | Backward 前需 AG？ |
|------|------------------------|-----------|-------------------|
...
**Forward：**
- CPU 线程在 layer i 的 forward 前发起 `all-gather(i+1)`，进入独立 CUDA stream
...
BwdStk `pop(-1)`（从末尾 pop）。时序 = pop 顺序 = list 末尾→开头。

期望时序（FULL_SHARD）：
`wait_AG(第1) → post_AG(第2) → compute(第3) → post_RS(第4=最后)`
...
1. **`reshard_after_forward` 默认值 = `True`**（FULL_SHARD，FSDP2 默认）。
   当前行为隐式建模了 SHARD_GRAD_OP；改为默认 True 会改变现有 layer-wise
   的 end_t —— 接受。
2. **删除 `wait RS(next)` 同步点** —— 接受。RS 纯异步 post，无层间 wait。
   完成同步仅在 optimizer step 前。
3. **RS 完成同步：显式** —— 在 OptimizerSimulator tail（layer-wise 模式）
   中增加 `async_wait_collective`，在 `optim_step` 之前。
4. **显式预取 2+ 层：现在就做** —— `fsdp_prefetch_layers` 配置字段，默认 1。
   Phase 1 和 Phase 2 在本轮一起实现。
